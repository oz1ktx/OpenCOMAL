...
bool isHexToken(const std::string& token, std::size_t expectedDigits)
{
...
std::string trimRightSpaces(const std::string& s)
{
...
std::string parseFcbFileName(const Z80Machine& machine, std::uint16_t fcbAddr)
{
    std::string name;
    name.reserve(12);
...
bool openFcbFile(Z80Machine& machine, std::uint16_t fcbAddr)
{
...
bool closeFcbFile(Z80Machine& machine, std::uint16_t fcbAddr)
{
    return machine.openFiles.erase(fcbAddr) > 0;
}

std::uint8_t sequentialReadFcbFile(Z80Machine& machine, std::uint16_t fcbAddr)
{
...
std::uint8_t popConsoleInputChar(Z80Machine& machine)
{
...
std::uint8_t z80FetchOpcode(void* context, zuint16 address)
{
...
std::uint8_t z80Read(void* context, zuint16 address)
{
    auto* machine = static_cast<Z80Machine*>(context);
    return machine ? machine->memory[address] : 0;
}

void z80Write(void* context, zuint16 address, zuint8 value)
{
    auto* machine = static_cast<Z80Machine*>(context);
    if (machine) {
...
std::uint8_t z80In(void*, zuint16)
{
    return 0xFF;
}

void z80Out(void*, zuint16, zuint8)
{
}

void z80Halt(void* context, zuint8 signal)
{
    auto* machine = static_cast<Z80Machine*>(context);
...
std::optional<int> parseLineNumberFromMessage(const QString& line)
{
...
AssemblerDiagnostic parseAssemblerDiagnostic(const QString& rawLine)
{
...
std::uint8_t memoryByte(const Z80Machine& machine, std::uint16_t address)
{
    return machine.memory[address];
}

std::uint16_t memoryWord(const Z80Machine& machine, std::uint16_t address)
{
...
DecodedInstruction decodeInstruction(const Z80Machine& machine, std::uint16_t address)
{
    ...
    case 0x10: {
        ...
        const std::uint16_t target = static_cast<std::uint16_t>(address + 2 + offset);
...
std::vector<Z80DisassemblyLine> buildDisassembly(const Z80Machine& machine,
                                                 ...
                                                 const Z80SourceMap* sourceMap)
{
    ...
    if (sourceMap && !sourceMap->orderedAddresses.empty()) {
        ...
        for (std::size_t index = startIndex; index < endIndex; ++index) {
            ...
            const std::vector<std::uint8_t>& bytes = entry->bytes.empty() ? fallback.bytes : entry->bytes;
...
Z80DebugSnapshot buildZ80DebugSnapshot(const Z80Machine& machine, const Z80SourceMap* sourceMap)
{
...
RunResult runZ80ComProgram(const QString& programPath,
                           const QString& consoleInput,
                           const QString& hostDirectory,
                           const Z80SourceMap* sourceMap,
                           IExecutionControl* executionControl)
{
    ...
    if (!hostDirectory.trimmed().isEmpty()) {
        machine.hostBaseDir = std::filesystem::path(hostDirectory.toStdString());
    } else {
    ...
    if (machine.exitReason == Z80ExitReason::InstructionLimit) {
        ...
        result.finished = false;
        std::ostringstream msg;
        msg << "Z80 execution stopped after reaching instruction limit (" << kMaxInstructions << ").";
        result.errorMessage = QString::fromStdString(msg.str());
        return result;
    }
...
AssemblerResult SjasmplusAssembler::assembleFile(const QString& sourcePath, const QString& outputDirectory)
{
    ...
    for (const QString& rawLine : lines) {
        ...
        result.diagnostics.push_back(parseAssemblerDiagnostic(rawLine));
    }
...
RunResult Z80CpmBackend::run(const RunRequest& request) const
{
    const QString lowerPath = request.programPath.toLower();
    if (lowerPath.endsWith(".com")) {
        return runZ80ComProgram(
            request.programPath, request.consoleInput, request.hostDirectory, nullptr, request.executionControl);
    }
