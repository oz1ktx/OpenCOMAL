...
inline std::vector<KeywordDocEntry> defaultKeywordDocEntries() {
    return {
        ...
        {"SPAWN", "Parallel and Queue", "Start a CLOSED PROC asynchronously. Forms: SPAWN proc(args...) and SPAWN handle: proc(args...)."},
        {"WAIT", "Parallel and Queue", "Wait for spawned workers. Forms: WAIT (all) or WAIT handle."},
        {"STOP", "Runtime and Errors", "Halt program execution. Extended form: STOP SPAWN handle requests cooperative stop of one spawned worker."},
