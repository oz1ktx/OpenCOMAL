					  ...
					  struct dim_ension *root)
{
	struct dim_list *work =
	    PARS_ALLOC(struct dim_list);

	work->id = id;
	work->strlen = strlen;
	work->dimensionroot = PARS_REVERSE(struct dim_ension, root);
...
#define GETEXP(x) struct expression *work = static_cast<struct expression *>(mem_alloc(PARSE_POOL, sizeof(int) + sizeof(enum optype) + (x)))
				      ...
				      struct exp_list *exproot)
{
	...
	work->op = op;
	work->e.expid.id = id;
	work->e.expid.exproot = PARS_REVERSE(struct exp_list, exproot);
				       ...
				       struct two_exp *twoexp)
{
	...
	work->op = stringidSYM;
	work->e.expsid.id = id;
	work->e.expsid.exproot = PARS_REVERSE(struct exp_list, exproot);

	if (!twoexp)
		work->e.expsid.twoexp = NULL;
	else {
		work->e.expsid.twoexp = PARS_ALLOC(struct two_exp);
...
PUBLIC int pars_handle_error_silent()
{
	int i = pars_error_happened;
...
PUBLIC int pars_last_error_pos()
{
	return pars_error_happened;
}
